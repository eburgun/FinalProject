...
namespace Helpers
{
	...
	int repeat_until_in_range(int min, int max, string strPrompt, int n_tries)
	{
		...
		throw out_of_range("Didn't enter a valid value within the given parameters.");
	}
	...
	vector<pair<int, float>> sort_cosines(float* cosines, int length, int k)
	{
		...
		for (int i = 0; i < length; i++)
		{
			...
			if (val > 0)
			{
				...
				{
					sorted_cosines.push_back(pair<int, float>(i, val));
				}
				else //if vector isn't full then replace lowest value
				{
					int lowest_ind = -1;
					float lowest_val = std::numeric_limits<float>::max();
					for (int j = 0; j < k; j++)
					{
						if (sorted_cosines[j].second < lowest_val)
						{
							lowest_ind = j;
							lowest_val = sorted_cosines[j].second;
						}
					}
					//if the lowest_ind changed and proposed val is greater than lowest val
					if (lowest_ind != -1 && val > lowest_val)
					{
						sorted_cosines[lowest_ind].first = i;
						sorted_cosines[lowest_ind].second = val;
					}
				}
			}
